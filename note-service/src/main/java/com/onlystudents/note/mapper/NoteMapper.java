...
public interface NoteMapper extends BaseMapper<Note> {
    ...
    @Select("SELECT * FROM note WHERE deleted = 0 AND status = 2 AND visibility = 0 ORDER BY hot_score DESC LIMIT #{limit}")
    List<Note> selectHotNotes(@Param("limit") Integer limit);
    
    @Select("SELECT * FROM note WHERE deleted = 0 AND status = 2 AND visibility = 0 ORDER BY publish_time DESC LIMIT #{limit}")
     ...
     */
    @Select("SELECT * FROM note WHERE deleted = 0 AND status = 2 AND visibility = 0 ORDER BY id LIMIT #{offset}, #{limit}")
