...
public class NoteController {
    ...
    public Result<NoteDTO> updateNote(@PathVariable Long noteId,
    ...
    public Result<NoteDTO> getNoteDetail(@PathVariable Long noteId,
    ...
    public Result<Void> publishNote(@PathVariable Long noteId,
                                     @RequestHeader(CommonConstants.USER_ID_HEADER) Long userId) {
    ...
    public Result<List<NoteDTO>> getHotNotes(@RequestParam(defaultValue = "20") Integer limit) {
        return Result.success(noteService.getHotNotes(limit));
    }
    
    @GetMapping("/latest")
    @Operation(summary = "最新笔记", description = "按时间排序获取最新笔记")
    public Result<List<NoteDTO>> getLatestNotes(@RequestParam(defaultValue = "20") Integer limit) {
        return Result.success(noteService.getLatestNotes(limit));
    }
    
    @GetMapping("/user/{userId}")
    @Operation(summary = "用户笔记列表", description = "获取指定用户的所有笔记")
    public Result<List<NoteDTO>> getUserNotes(@PathVariable Long userId) {
