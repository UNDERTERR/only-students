...
public interface NoteStatsMapper extends BaseMapper<NoteStats> {
    ...
    @Select("SELECT * FROM note_stats WHERE creator_id = #{creatorId} ORDER BY heat_score DESC")
    ...
    @Update("UPDATE note_stats SET heat_score = #{heatScore}, ranking = #{ranking}, updated_at = NOW() WHERE note_id = #{noteId}")
